...
import fit.wenchao.lanchat.model.JsonResult;
import fit.wenchao.lanchat.utils.ClassUtils;
...
public class GlobalExceptionHandler {
    ...
    public JsonResult otherException(HttpServletResponse req, Exception ex) {
        ...
        if (ex instanceof MaxUploadSizeExceededException) {
            return this.errorCodeException(req,
                    new BackendException(null, RespCode.UPLOAD_FILE_SIZE_EXCEED_UPPER_LIMIT));
        }
        ...
        jsonResult
                .setMsg("Server internal error occurred --> [ " + ex.getClass().getTypeName() + "-->" + ex.getMessage()
                        + " ]");
        return jsonResult;
    }
