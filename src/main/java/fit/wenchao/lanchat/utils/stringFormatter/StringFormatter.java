...
public class StringFormatter {
    ...
    private static void findAndStorePairQuotes(int currentIdx, String format,
            Map<Integer, Integer> singleQuotesIndexMap, List<Entry> singleQuotesEntryList) {
        if (SINGLE_QUOTE == format.charAt(currentIdx)) {
            if (!singleQuotesIndexMap.containsKey(currentIdx) && !singleQuotesIndexMap.containsValue(currentIdx)) {
                ...
                for (int anotherSingleQuoteIndex = currentIdx + 1; anotherSingleQuoteIndex < format
                        .length(); anotherSingleQuoteIndex++) {
